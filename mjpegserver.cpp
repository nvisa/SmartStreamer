...
{
	if (buf.getMimeType() == "image/jpeg")
		return processJpegBuffer(buf);
	...
	im.save(&qbuf, "JPG");
	RawBuffer buf2("image/jpeg", (const void *)ba.constData(), ba.size());
	return processJpegBuffer(buf2);
}

int MjpegElement::processJpegBuffer(const RawBuffer &buf)
{
